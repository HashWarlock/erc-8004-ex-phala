...
class TestIdentityRegistry:
    ...
    def test_registration_fee(self, w3, deployed_contracts, contract_abis, blockchain_snapshot):
    ...
    def test_agent_registration(
        self, w3, deployed_contracts, contract_abis, test_accounts, blockchain_snapshot
    ):
        ...
        try:
            ...
            already_registered = existing_info[0] > 0
        except Exception as e:
            # AgentNotFound error is expected if not registered
            already_registered = False
            existing_info = (0, "", "0x0000000000000000000000000000000000000000")

        if not already_registered:  # Not registered
            # Build registration transaction
            nonce = w3.eth.get_transaction_count(test_account["address"])
            tx = identity_contract.functions.newAgent(
                test_domain, test_account["address"]
            ).build_transaction(
                {
            ...
            # Sign and send
            signed_tx = w3.eth.account.sign_transaction(tx, test_account["private_key"])
    ...
    def test_get_agent_count(self, w3, deployed_contracts, contract_abis, blockchain_snapshot):
...
class TestReputationRegistry:
    ...
    def test_feedback_authorization(
        self, w3, deployed_contracts, contract_abis, alice_account, bob_account, blockchain_snapshot
    ):
        ...
        try:
            ...
            # Agent not found - skip test
            pytest.skip("Bob not registered")
        ...
        if not is_authorized:
            ...
            if receipt.status == 1:
                ...
                is_authorized, auth_id = (
                    reputation_contract.functions.isFeedbackAuthorized(
                        bob_id, alice_id
                    ).call()
                )
...
class TestValidationRegistry:
    ...
    def test_validation_request(
        ...
        signed_tx = w3.eth.account.sign_transaction(tx, alice_account["private_key"])
        tx_hash = w3.eth.send_raw_transaction(signed_tx.raw_transaction)
        receipt = w3.eth.wait_for_transaction_receipt(tx_hash)
