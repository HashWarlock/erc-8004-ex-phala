"""
TEE-Enabled Server Agent - Market Analysis Service

This agent demonstrates a TEE-enabled Server Agent role in the ERC-8004 ecosystem.
It uses CrewAI to perform market analysis tasks and submits its work
for validation through the ERC-8004 registries, with keys derived from the TEE.
"""
...
class TEEServerAgent(ERC8004TEEAgent):
    """
    TEE-enabled Server Agent that provides market analysis services
    """
    
    def __init__(self, agent_domain: str, salt: str, tee_endpoint: Optional[str] = None):
        """Initialize the TEE Server Agent"""
        super().__init__(agent_domain, salt, tee_endpoint)
        
        # Initialize CrewAI components
        self._setup_crew()
        
        print(f"🤖 TEE Server Agent initialized")
    ...
    def perform_market_analysis(self, symbol: str, timeframe: str = "1d") -> Dict[str, Any]:
        ...
        # Include TEE attestation in the analysis package
...
"""
    ...
    def get_trust_models(self) -> list:
        ...
        return ["feedback", "inference-validation", "tee-attestation"]
    
    def get_agent_card(self) -> Dict[str, Any]:
        """Generate AgentCard following A2A specification"""
        return {
            "agentId": self.agent_id,
            "name": "TEE-Enabled Market Analysis Server Agent",
            "description": "Provides comprehensive cryptocurrency market analysis using AI with TEE security",
            "version": "1.0.0",
            "skills": [
                {
