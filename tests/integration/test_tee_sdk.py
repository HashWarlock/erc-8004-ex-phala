    ...
    try:
            ...
            if hasattr(info, 'tcb_info'):
                ...
                if hasattr(info.tcb_info, 'event_log') and info.tcb_info.event_log:
                    ...
                    if len(info.tcb_info.event_log) > 0:
                        print(f"     First Event: {info.tcb_info.event_log[0].event if hasattr(info.tcb_info.event_log[0], 'event') else 'N/A'}")
        ...
        try:
            ...
            if hasattr(key_result, 'certificate_chain'):
                ...
                cert_preview = key_result.certificate_chain[:100] if key_result.certificate_chain else "N/A"
        ...
        try:
            ...
            if hasattr(quote_result, 'quote'):
                ...
                quote_preview = quote_result.quote[:100] if quote_result.quote else "N/A"
        ...
        try:
            ...
            for i in range(3):
                ...
                if hasattr(key_result, 'key'):
                    key_identifier = key_result.key[:50] if key_result.key else f"key_{i}"
                else:
            ...
            if len(keys) == len(set(keys)):
                ...
                print(f"{YELLOW}⚠️  Some keys are identical (check key derivation){RESET}")
