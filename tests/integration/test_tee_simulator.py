...
def tee_client():
    ...
    if not os.path.exists(socket_path):
        pytest.skip(
            f"TEE simulator socket not found at {socket_path}. Run 'make tee-start' first."
        )
...
class TestTEESimulator:
    ...
    def test_quote_with_different_data(self, tee_client):
        ...
        quotes = []
        test_data = ["data1", "data2", "completely-different"]
        ...
        assert len(set(quotes)) == len(quotes), (
            "Quotes should differ for different data"
        )
    ...
    def test_key_derivation_consistency(self, tee_client):
        ...
        assert key1.signature_chain == key2.signature_chain, (
            "Same ID should produce same signatures"
        )
