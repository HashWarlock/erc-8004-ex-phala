...
from tests.test_utils import SimulatorManager, get_test_web3, get_test_tee_client
...
def simulator_manager():
    """Simulator manager for test session"""
    manager = SimulatorManager()
    
    # Ensure Anvil is running
    success, error = manager.ensure_simulators_running(require_tee=False)
    if not success:
        pytest.skip(error)
    ...
    if not os.path.exists("deployed_contracts.json"):
        if not manager.deploy_contracts():
            pytest.skip("Failed to deploy contracts")
    
    yield manager
    
    # Cleanup is handled by manager if needed
...
def blockchain_snapshot(w3, deployed_contracts):
    """
    Create a blockchain snapshot at the start of each test class.
    This ensures test isolation between different test classes.
    """
    # Skip if w3 is not available
    if not w3 or not deployed_contracts:
        yield
        return
    
    # Take a snapshot after contracts are deployed
    snapshot_id = w3.provider.make_request("evm_snapshot", [])
    
    # Provide the snapshot ID to tests
    yield snapshot_id["result"]
    
    # Revert to the snapshot after the test class
    try:
        w3.provider.make_request("evm_revert", [snapshot_id["result"]])
    except:
        # If revert fails, it's okay - we're done with this class anyway
        pass
...
def tee_client(simulator_manager):
    """TEE client connected to real simulator"""
    # Ensure TEE is running
    success, error = simulator_manager.ensure_simulators_running(require_tee=True)
    if not success:
        pytest.skip(error)
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "unit: Unit tests that don't require blockchain")
