...
def test_tee_key_derivation():
    ...
    for domain, salt in test_cases:
        ...
        try:
            ...
            assert False, f"Failed to create agent: {e}"
    ...
    if agents[0].address != agents[1].address:
        ...
        assert False, "Uniqueness check failed"
...
def test_tee_attestation():
    ...
    try:
        ...
        if "error" in attestation:
            print(
                f"⚠️  Attestation failed (expected in simulator): {attestation['error']}"
            )
        ...
        # Test completed successfully  # Don't fail the test in simulator
...
def test_tee_vs_traditional():
    ...
    traditional = ERC8004BaseAgent(
        agent_domain="traditional.example.com", private_key=private_key
    )
