...
class ERC8004BaseAgent:
    ...
    def _check_registration(self):
        ...
        try:
            ...
            if result[0] > 0:  # AgentID > 0 means registered
                # Verify the domain matches what we expect
                stored_domain = result[1]  # agentDomain is the second field
                if stored_domain == self.agent_domain:
                    self.agent_id = result[0]
                    print(f"✅ Agent already registered with ID: {self.agent_id}")
                else:
                    # Address is registered but with a different domain
            ...
            # AgentNotFound error (0xe93ba223) is expected when agent doesn't exist
            error_str = str(e)
    ...
    def register_agent(self) -> int:
        ...
        required = self.w3.to_wei(0.01, "ether")  # 0.005 for fee + gas
        if balance < required:
            raise ValueError(
