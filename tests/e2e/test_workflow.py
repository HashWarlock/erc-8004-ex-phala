...
class TestCompleteWorkflow:
    ...
    def test_full_market_analysis_workflow(
        self, w3, alice_account, bob_account, charlie_account
    ):
        """
        ...
        # Use unique domains and keys with timestamp to avoid conflicts
        import time
        import secrets
        timestamp = int(time.time() * 1000)
        
        # Generate unique private keys for this test
        alice_key = "0x" + secrets.token_hex(32)
        bob_key = "0x" + secrets.token_hex(32)
        charlie_key = "0x" + secrets.token_hex(32)
        
        # Alice as Server Agent
        alice = ServerAgent(
            agent_domain=f"alice{timestamp}.market.analysis.com",
            private_key=alice_key,
        )
        
        # Fund the new accounts
        from tests.test_utils import fund_account
        fund_account(w3, alice.address, 1)
        if not alice.agent_id:
            try:
                alice_id = alice.register_agent()
                print(f"✅ Alice registered with ID: {alice_id}")
            except Exception as e:
                # If registration fails, try with a different domain
                if "AddressAlreadyRegistered" in str(e) or "0x2d42c772" in str(e):
                    print(f"⚠️  Address already registered, using existing registration")
                    # Get the existing registration
                    from web3 import Web3
                    w3_local = Web3(Web3.HTTPProvider("http://127.0.0.1:8545"))
                    # Just use the existing agent with its original domain
                    # We can't re-register with same address but different domain
                    alice.agent_id = 1  # Will be overridden by _check_registration
                    alice._check_registration()
                    if not alice.agent_id:
                        raise Exception("Could not determine alice agent ID")
                else:
                    raise
        else:
        ...
        bob = ValidatorAgent(
            agent_domain=f"bob{timestamp}.validator.service.com",
            private_key=bob_key,
        )
        fund_account(w3, bob.address, 1)
        if not bob.agent_id:
        ...
        charlie = ERC8004BaseAgent(
            agent_domain=f"charlie{timestamp}.client.app.com",
            private_key=charlie_key,
        )
        fund_account(w3, charlie.address, 1)
        ...
        print(
            f"✅ Validation completed with score: {validation_result.get('validation_score', 0)}/100"
        )
    ...
    def test_trust_model_verification(self, w3, alice_account, bob_account):
        ...
        # Use unique domains with timestamp
        import time
        timestamp = int(time.time() * 1000)
        
        alice = ServerAgent(
            agent_domain=f"alice{timestamp}.trust.test.com",
            private_key=alice_account["private_key"],
        )

        bob = ValidatorAgent(
            agent_domain=f"bob{timestamp}.trust.test.com", private_key=bob_account["private_key"]
        )
    ...
    def test_multi_agent_interaction(self, w3, test_accounts):
        ...
        # Use unique domains and keys with timestamp
        import time
        import secrets
        timestamp = int(time.time() * 1000)

        # Create 3 agents with unique keys
        for i in range(3):
            private_key = "0x" + secrets.token_hex(32)
            agent = ERC8004BaseAgent(
                agent_domain=f"agent{i}_{timestamp}.multitest.com",
        ...
        for i in range(len(agents) - 1):
            try:
                ...
                print(
                    f"✅ Agent {agents[i].agent_id} authorized feedback from Agent {agents[i + 1].agent_id}"
                )
            except Exception as e:
                print(
                    f"ℹ️  Authorization between {agents[i].agent_id} and {agents[i + 1].agent_id}: {str(e)[:30]}..."
                )
